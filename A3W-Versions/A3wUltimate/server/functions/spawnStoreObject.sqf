...
_isktttStore = ["ktttStore", _marker] call fn_startsWith;

if (_key != "" && isPlayer _player && {_isGenStore || _isGunStore || _isVehStore || _isktttStore}) then
{
	_timeoutKey = _key + "_timeout";
	_objectID = "";
	_playerSide = side group _player;

	if (_isGenStore || _isGunStore || _isktttStore) then
	{
		...
		switch (true) do
		{
			...
			case _isktttStore: { _objectsArray = ktttObjectsArray };
		};
