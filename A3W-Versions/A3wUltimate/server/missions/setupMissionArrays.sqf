...
[
	...
	["mission_HostilePlaneFormation", 1],
	["mission_APC", 1],
	["mission_MBT", 1],
...
RareMissions =
[
	["mission_Artillery", 0.1],
	["mission_WepDeal", 0.5], 
	["mission_InfGroup", 1],
	["mission_ScopedSquad", 0.7],
	["mission_ConFile", 1],
	["mission_DrugDeal", 0.1]
];
...
RareMissions = [RareMissions] call removeDisabledMissions;

{ _x set [2, false] } forEach MainMissions;
{ _x set [2, false] } forEach SideMissions;
{ _x set [2, false] } forEach MoneyMissions;
{ _x set [2, false] } forEach RareMissions;
{ _x set [2, false] } forEach ExchangeMissions;

MissionSpawnMarkers = [];
SunkenMissionMarkers = [];
ScopedMissionMarkers = [];

{
	switch (true) do
	{
		...
		case (["ScopedMission_", _x] call fn_startsWith):
		{
			ScopedMissionMarkers pushBack [_x, false];
		};
	};
