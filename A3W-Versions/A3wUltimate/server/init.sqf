	...
	if (_playerSavingOn) then
	{
		...
		if (_savingMethod != "profile") then
		{
			_setupPlayerDB spawn
			{
				...
				{ [getPlayerUID _x, name _x] call fn_kickPlayerIfFlagged } forEach (call allPlayerz);
			};
		};
	};
...
[] spawn
{
	...
	{
		if (!isPlayer _x && {[["GenStore","GunStore","VehStore","ktttStore"], vehicleVarName _x] call fn_startsWith}) then
		{
