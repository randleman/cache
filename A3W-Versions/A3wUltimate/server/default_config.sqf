...
A3W_serverSavingInterval = 15*60;   // Interval in seconds between automatic vehicle & object saves; should be kept at 1 min for profileNamespace and iniDB, while for extDB it can be relaxed to 3-5 mins
...
A3W_rareMissionDelay = 1*60;      // Time in seconds between Rare Missions
A3W_rareMissionTimeout = 60*60;    // Time in seconds that a Rare Mission will run for, unless completed
A3W_exchangeMissionDelay = 20*60;  // Time in seconds between Exchange Missions
A3W_exchangeMissionTimeout = 20*60;// Time in seconds that a Exchange Mission will run for, unless completed