...
#include "dialog\ktttstoreDefines.sqf";
...
storePurchaseHandle = _this spawn
{
	...
	_dialog = findDisplay ktttstore_DIALOG;
	_itemlist = _dialog displayCtrl ktttstore_item_list;
	_totalText = _dialog displayCtrl ktttstore_total;
	_playerMoneyText = _Dialog displayCtrl ktttstore_money;

	_itemIndex = lbCurSel ktttstore_item_list;
	...
	{
		{
			if (_itemText == _x select 0 && _itemData == _x select 1) exitWith
			{
				...
				switch (_x select 3) do
				{
					...
					// Crates transferred to ktttObjectsArray below
					/*case "ammocrate":
					{
	...
	{
		{
			if (_itemData == _x select 1) exitWith
			{
				...
				[] execVM "client\systems\ktttStore\getInventory.sqf";
			};
		} forEach (call ktttPlayerItems);
	};
	...
	if (isNil "_price") then
	{
		...
		} forEach (call ktttBackpackArray);
	};
